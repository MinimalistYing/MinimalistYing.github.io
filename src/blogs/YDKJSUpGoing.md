...
可以用来组织链接挑战，当然现在不建议这么使用，只是大家在互联网上肯定还是能见到这种写法，能明白为什么就行：
```html
...
console.log(a)// Uncaught ReferenceError: a is not defined
```
变量提升不是一个应该值得利用的特性，我们应该在函数的开头声明好所有需要的变量，相反的函数声明的提升是一个更被大家所接受的特性。
...
此外，函数表达式可以是匿名的，并且哪怕它被命名，它的命名也只在自身的函数作用域内可见：
```js
